...
- Diff search query and next/previous match navigation now also run through that shared workspace-editor/session ordering and the Review workspace layout’s excerpt order, instead of remaining a Files-only behavior. That is the first concrete shared editor behavior on the new Diff workspace-editor session beyond path selection.
