...
- Review compare apply now also seeds the full shared projected display-row cache from those persistent side editors up front, so revisiting or first-scrolling the loaded Diff surface stays inside one retained editor-display map instead of forcing fresh viewport projection on the first pass through each range.
- Review surface refresh no longer projects new viewport slices from the side editors during scroll. It now consumes the retained shared display-row cache and only falls back to one explicit full-cache rebuild when that retained projection is missing, which keeps runtime scrolling closer to Zed's persistent multibuffer/display-map behavior.
- Diff-mode search query updates now explicitly invalidate and rebuild that retained shared display-row cache, so search highlights and projected display rows stay on the same persistent projection path instead of depending on scroll-time reprojection.
- Review surface refresh now also reads retained display-row coverage by reference instead of cloning the full shared cache on each viewport refresh, which removes another hot-path copy from Diff scrolling on the way to the 8ms frame budget.
...
- The sticky Review file banner now also paints and hit-tests inside the shared viewport element itself instead of mounting as a separate subtree above the scroller, which follows Zed's editor-owned buffer-header pattern more closely and leaves fewer non-editor-owned Review UI seams.
...
- Review workspace sessions now also retain a display-row geometry foundation alongside the legacy raw-row geometry, seeded from the shared Review display rows. That gives the next phase a session-owned source for per-row display spans, section display spans, and expanded pixel heights without switching the live surface over prematurely.
- Review surface snapshots now also position visible rows, section pixel ranges, total surface height, and top-visible display-row state from that display-row geometry foundation, so wrapped or display-expanded Review rows no longer collapse back onto one raw-row pixel slot while painting or computing visible state.
- Review now also requests projected rows from the persistent side editors in display-row space instead of raw-row space, so the shared editor path can return the full wrapped/expanded viewport slice that the Review surface geometry expects.
- `ReviewWorkspaceSession` no longer keeps a second raw-row geometry cache in parallel with that display-geometry model; section pixel ranges, row boundaries, and total surface height now have one owner inside the shared session state.
- `ReviewWorkspaceSession` now also retains and merges previously projected display rows across viewport refreshes, so revisiting an already-covered Review range can reuse session-owned display/syntax rows instead of rebuilding that slice from the hidden side editors every time.
...
- Review surface visible-state and viewport hit-testing now also carry display-row identity alongside raw row identity, so the shared surface can reason about top/visible editor display rows directly instead of re-deriving everything from raw row starts before dispatching input or sticky-header state.
...
- Review workspace display-row entries now also preserve multi-row raw workspace ranges from projected side-editor rows, and the shared display-geometry model treats those covered-followup raw rows as zero-height spans instead of forcing them back into one-row-per-raw-row layout.
