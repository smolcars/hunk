...
- Embedded video support depends on the bundled CEF build's media codec flags. The current prebuilt CEF runtime is acceptable for general browsing and WebM/VPx-style playback, but sites that embed MP4/H.264 or AAC-backed media may fail with Chromium demuxer errors such as `DEMUXER_ERROR_NO_SUPPORTED_STREAMS`. Treat full H.264/AAC support as a future runtime decision: ship or build a CEF runtime with proprietary codec support (`proprietary_codecs=true`, `ffmpeg_branding=Chrome`) only after accepting the licensing and distribution implications.
