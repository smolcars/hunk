...
fn ai_exploration_tool_name_for_item(item: &hunk_codex::state::ItemSummary) -> Option<String> {
    ...
    match thread_item {
        hunk_codex::protocol::ThreadItem::DynamicToolCall { tool, .. }
        | hunk_codex::protocol::ThreadItem::McpToolCall { tool, .. } => Some(tool),
        _ => None,
    }
}
...
fn ai_collaboration_group_summary_for_item(
    ...
    let thread_item = ai_timeline_item_thread_item(item)?;
    let hunk_codex::protocol::ThreadItem::CollabAgentToolCall {
        tool,
        receiver_thread_ids,
        ..
    } = thread_item
    ...
    match tool {
        hunk_codex::protocol::CollabAgentTool::SpawnAgent => summary.spawned = 1,
        hunk_codex::protocol::CollabAgentTool::SendInput => summary.sent_inputs = 1,
        hunk_codex::protocol::CollabAgentTool::ResumeAgent => summary.resumed = 1,
        hunk_codex::protocol::CollabAgentTool::Wait => summary.waits = 1,
        hunk_codex::protocol::CollabAgentTool::CloseAgent => summary.closed = 1,
    }
    summary
        .receiver_thread_ids
...
fn ai_file_change_batch_group_summary_for_item(
    ...
    let thread_item = ai_timeline_item_thread_item(item)?;
    let hunk_codex::protocol::ThreadItem::FileChange { changes, .. } = thread_item else {
        return None;
    };
...
fn ai_timeline_item_thread_item(
    ...
    let details_json = item
        ...
        .map(str::trim)
        .filter(|value| !value.is_empty())?;
    serde_json::from_str::<hunk_codex::protocol::ThreadItem>(details_json).ok()
}
