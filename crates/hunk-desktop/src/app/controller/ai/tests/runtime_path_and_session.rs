...
#[test]
fn review_mode_selected_path_preserves_explicit_selection() {
...
fn workspace_mad_max_mode_reads_per_workspace_flags() {
    let state = AppState {
        ...
        preferred_project_open_target_id: None,
...
fn normalized_ai_session_selection_preserves_selection_while_models_load() {
    assert_eq!(
        ...
        (Some("gpt-5.3-codex".to_string()), Some("high".to_string()),),
    );
}
...
fn normalized_ai_session_selection_preserves_model_default_effort_when_unset() {
    ...
    assert_eq!(
        normalized_ai_session_selection(models.as_slice(), Some("gpt-5.3-codex".to_string()), None,),
...
fn command_name_without_path_detection_is_stable() {
    ...
    assert!(!is_command_name_without_path(std::path::Path::new(
        "./codex"
    )));
    assert!(!is_command_name_without_path(std::path::Path::new(
        "/usr/bin/codex"
...
fn bundled_codex_resolution_picks_existing_runtime_candidate() {
    ...
    std::fs::create_dir_all(runtime_path.parent().expect("runtime parent should exist"))
...
#[test]
fn workspace_codex_resolution_prefers_repo_asset_for_cargo_targets() {
    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("clock should be monotonic")
        .as_nanos();
    ...
    let resolved = resolve_workspace_codex_executable_from_exe(exe_path.as_path());
    assert_eq!(resolved, Some(runtime_path));
...
#[test]
fn workspace_codex_resolution_ignores_packaged_bundle_paths() {
    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("clock should be monotonic")
        .as_nanos();
...
#[cfg(target_os = "linux")]
#[test]
fn bundled_codex_resolution_supports_packaged_linux_launcher_binary_names() {
    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("clock should be monotonic")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("hunk-codex-runtime-bin-{unique}"));
    let exe_path = root.join("usr").join("bin").join("hunk_desktop_bin");
    std::fs::create_dir_all(
        exe_path
            .parent()
            .expect("exe parent directory should exist"),
...
fn normalized_user_input_answers_defaults_to_first_option_or_blank() {
    ...
    assert_eq!(answers.get("q-option"), Some(&vec!["first".to_string()]));
...
fn normalized_user_input_answers_preserves_existing_answers() {
    ...
    assert_eq!(answers.get("q-option"), Some(&vec!["custom".to_string()]));
}

#[test]
fn supported_image_path_check_is_case_insensitive() {
    assert!(is_supported_ai_image_path(std::path::Path::new(
        "image.PNG"
    )));
    assert!(is_supported_ai_image_path(std::path::Path::new(
        "shot.JpEg"
