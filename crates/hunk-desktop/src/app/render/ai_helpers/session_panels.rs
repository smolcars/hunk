...
struct AiSessionControlsPanelView<'a> {
    models: &'a [hunk_codex::app_server_protocol::Model],
...
fn ai_account_summary(
    account: Option<&hunk_codex::app_server_protocol::Account>,
    ...
    match account {
        Some(hunk_codex::app_server_protocol::Account::ApiKey { .. }) => {
            "Signed in with API key.".to_string()
        }
        Some(hunk_codex::app_server_protocol::Account::Chatgpt { email, plan_type }) => {
...
fn ai_rate_limit_windows(
    snapshot: &hunk_codex::app_server_protocol::RateLimitSnapshot,
) -> (
    Option<hunk_codex::app_server_protocol::RateLimitWindow>,
    Option<hunk_codex::app_server_protocol::RateLimitWindow>,
) {
...
fn ai_model_picker_label(
    models: &[hunk_codex::app_server_protocol::Model],
...
fn ai_effort_picker_label(
    ...
    selected_model: Option<&hunk_codex::app_server_protocol::Model>,
) -> String {
